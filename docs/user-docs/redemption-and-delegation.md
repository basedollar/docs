...
Redemptions serve the crucial purpose of keeping BD pegged to the value of a dollar, creating a price floor around $0.9945. They do this in a decentralized way without reliance on centralized assets, oracles, or 3rd parties.

A redemption is essentially swapping BD for collateral at face value, as if 1 BD is exactly worth $1.00. Redemptions can be initiated by anyone, but are only profitable when BD is less than $1.

**Important**: The redeemer sends BD to the protocol and receives collateral from branches registered as redeemable, minus the redemption fee. The current WETH, wstETH, rETH, cbBTC, cbETH, and AERO branches are redeemable.

:::warning
**Redeemability Is Configured Per Branch**

An LP-token branch may be registered as redeemable or non-redeemable; LP collateral is not automatically protected from redemptions.
:::
...
A redemption can occur at any time, but will likely only happen when it is profitable to do so. This is usually the case when the price of BD is less than $1 (minus the current redemption fee).

### Who can initiate a redemption?

Any Base address can initiate a redemption, provided that they have a sufficient amount of BD to do so. However, we expect redemptions to be mainly performed by professional bots rather than humans.
...
* Before the redemption: 10 ETH collateral, 20'000 BD debt.
* After the redemption: 5.025 ETH collateral, 5'000 BD debt.
...
Partially affected Troves whose debt stays above the minimum debt threshold of 200 BD continue to work as before, while Troves whose debt is reduced to a lesser amount (or 0) switch to a dormant operating mode (see below for [more](#what-happens-when-redemptions-cause-a-debt-of-a-trove-to-fall-below-the-minimum-amount) info).

### How do redemptions work using multiple collateral assets?

:::tip
**Only Registered Redeemable Branches Participate**

Normal redemptions use branches in the Collateral Registry's redeemable list. Non-redeemable branches are excluded.
:::
...
The split is dynamic, optimizing for the economic safety of the system among eligible redeemable branches.
...
To mitigate this risk, the system redeems proportionally to the "outside debt" of each eligible redeemable branch. This is calculated as the total debt borrowed against a specific collateral minus the size of the Stability Pool for that borrowing market.

Here is an example: given outside debt amounts of 100 BD, 50 BD and 100 BD respectively across wETH, cbBTC, and wstETH, a redemption will result in a 40% (wETH), 20% (cbBTC) and 40% (wstETH) split.
...
Upon each redemption of x BD: `baseRate` is decayed based on time passed since the last fee event and incremented by an amount proportional to the fraction of the total BD supply to be redeemed, i.e. `x/total_bd_supply`
...
For borrowers in a redeemable branch, the risk of redemption depends on two factors: the interest rate you set and the price of BD.

**The interest rate** you set determines how much BD must be redeemed before it's your turn. The higher your rate, the more BD is redeemable before you, and vice versa.
...
This means that 41M BD must be removed from the system before it would reach you. However, this number is relative, and you also need to consider recent redemption activity. While past events don't guarantee future outcomes, they can serve as a useful guide.

For example, if only 200K BD were redeemed in the last week, you're comparatively safer than if 15M were redeemed.

**The price of BD** is the second crucial factor. When it trades above $1, redemptions become unprofitable and should cease. If demand for BD is strong, it could maintain a price above $1 for an extended period.

During such times, you can comfortably reduce the interest rate you're paying without increasing your risk of redemption.

**Branch Choice**: A non-redeemable branch is excluded from normal Collateral Registry redemptions, regardless of whether its collateral is an LP token.
...
If the redeemed amount exceeds the debt of an affected Trove, it doesn't get closed as in Liquity V1, but remains open with 0 BD debt and the remaining collateral. The owner of a fully redeemed Trove may close it by withdrawing the remaining collateral, or borrow again to bring its debt above the minimum of 200 BD, topping up its collateral if needed.

In the scenario that the redeemed amount of a Trove does not exceed the debt of a Trove, but would leave it between 0 and 200 BD, the Trove would remain open with the remaining debt, and the remaining collateral. The owner of the Trove may close it by paying off the remaining debt and withdrawing the remaining collateral, or borrow anew as described above.

### How to redeem BD for collateral using the blockchain explorer

**Step 1**

To redeem BD you first have to give the CollateralRegistry contract an approval to use your BD using the approve() function of the BD token contract.
...
**Step 2**\
You can now redeem BD using the `CollateralRegistry` contract:

[(insert CollateralRegistry address after deployment)](#)Simply input the BD amount to redeem, the redemption fee percentage you are willing to accept and the maximum number of list iterations per collateral (limits the number of troves whose debt can be repaid on each branch).
